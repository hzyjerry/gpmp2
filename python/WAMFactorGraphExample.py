...
for i in range(0, total_time_step + 1):
    ...
    if i > 0:
        ...
        # graph_obs.add(ObstacleSDFFactorArm(key_pos, arm, sdf, cost_sigma, epsilon_dist))
        
        # # GP cost factor
        # if check_inter > 0:
        #     for j in range (0, int(check_inter)):
        #         tau = j * (total_time_sec / total_check_step)
        #         graph.add(ObstacleSDFFactorGPArm(key_pos1, key_vel1, key_pos2, key_vel2, arm, sdf, cost_sigma, epsilon_dist, Qc_model, delta_t, tau))
        #         graph_obs.add(ObstacleSDFFactorGPArm(key_pos1, key_vel1, key_pos2, key_vel2,arm, sdf, cost_sigma, epsilon_dist, Qc_model, delta_t, tau))
